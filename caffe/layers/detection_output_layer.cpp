...
void DetectionOutputLayer<Dtype>::Forward_cpu(
	...
	// float *out1 = bottom[0]->mutable_cpu_data();
	// for (int j=0; j < bottom[0]->count(); j++)
		// fprintf(pp1, "%f  -- 11\n", out1[j]);
	// fclose(pp1);
	
	// FILE* pp2 = fopen("DetectionOutput2.txt", "wb");
	// float *out2 = bottom[1]->mutable_cpu_data();
	// for (int j=0; j < bottom[1]->count(); j++)
		// fprintf(pp2, "%f  -- 22\n", out2[j]);
	// fclose(pp2);
	
	// FILE* pp3 = fopen("DetectionOutput3.txt", "wb");
	// float *out3 = bottom[2]->mutable_cpu_data();
	// for (int j=0; j < bottom[2]->count(); j++)
		// fprintf(pp3, "%f  -- 33\n", out3[j]);
	// fclose(pp3);
